...
\begin{align}
  ...
  M_{\text{bnd}} &= \mathrm{clip}\!\left(1 - \dfrac{a_{\text{outer}} - 4}{76},\, 0,\, 1
ight) \label{eq:mbnd}
\end{align}
...
\begin{align}
  ...
  M_{\text{maze}} &= \min\!\left(M_{\text{topology}},\, M_{\text{diversity}}
ight) \cdot M_{\text{wall\_gate}} \label{eq:final}
\end{align}

权重的设计动机: v2.1 起重新分配 (旧版 $M_b$/$M_s$/$M_j$ 各 0.10, $M_c$ 0.40, $M_{\text{bnd}}$ 0.30):
...
经验数据 (§5 中 v5+v6 sweep): 4 个 ES top 跑出来 $M_c \in [0.74, 0.98]$, 旧权重下几乎都被 clip 到 1.0; 新权重下 (a) manhattan-2 0.74, (b) chebyshev-1 0.98, 区分度恢复, (a) 从 #1 跌到 #3, (b) 升到 #1。
